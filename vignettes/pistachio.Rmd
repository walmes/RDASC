...
# List of packages.
pkg <- c("tidyverse", "agricolae", "emmeans", "lme4")
...
by a numerical optimation method.
...
assumption. These data could be analysed assuming a probability
distribution to count data. Here we decided to use a Gaussian model to
...
tb_means$cld <- c("a", "b")[c(2, 1, 1, 1, 1, 1, 1)]
...
tb_means$cld <- c("b", "a")[c(1, 2, 2, 2, 2, 2, 2)]
...
emm <- emmeans(m0, specs = ~iso)
tb_means <- multcomp::cld(emm, type = "response") %>%
    as.data.frame()
tb_means
...
gg_af <- ggplot(data = tb_means,
    ...
    geom_text(mapping = aes(label = sprintf("%0.2f%s", prob, cld)),
              ...
              nudge_y = 0.05,
              show.legend = FALSE) +
    labs(x = expression("Appressory germination"),
         y = "Isolates")
gg_af
```
...
```{r}
# ATTENTION: new data from 2019 are available.
...
# Each experimental codition has 3 trees and 10 clusters per tree.
ggplot(data = vv,
